...
void SPI_SetSpeed(u8 SPI_BaudRatePrescaler)
{
	SPI1_SetSpeed(SPI_BaudRatePrescaler);
}
...
{
	ENC28J60_CS = 0;
	SPI_ReadWriteByte(ENC28J60_READ_BUF_MEM);
	while(len)
	{
		len--;			  
		*data = (u8)SPI_ReadWriteByte(0);
		data++;
	}
	*data = '\0';
	ENC28J60_CS = 1;
...
{
	ENC28J60_CS = 0;
	SPI_ReadWriteByte(ENC28J60_WRITE_BUF_MEM);		 
	while(len)
	{
		len--;
		SPI_ReadWriteByte(*data);

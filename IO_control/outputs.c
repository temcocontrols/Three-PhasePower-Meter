...
//#endif			
			...
			{	
				if(outs->switch_status != switch_status_back[point])
				{
					if(point == 26) Test[34]++;
					if(switch_status_back[point] == 1)
					{// store control vluel
						control_auto[point] = outs->control;
						
					}
					if(outs->switch_status == 1)
					{
						outs->control = control_auto[point];
					}
					switch_status_back[point] = outs->switch_status;
				}
